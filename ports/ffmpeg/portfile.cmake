...
# --- FrameCapture overlay: BEGIN ------------------------------------------
# The only divergence from the upstream vcpkg ffmpeg port. SPEC.md §2.1 requires
# an LGPL build configured with `--disable-everything` plus an explicit component
# whitelist, and with network support removed at *compile* time so that the
# "no network egress from the engine" non-goal (SPEC.md §0.2) is enforced by the
# linker rather than by code review.
#
# Every entry below must be justified by a spec section. Do not add a component
# because "it might be useful" -- each one is attack surface and binary size.
include("${CMAKE_CURRENT_LIST_DIR}/framecapture-whitelist.cmake")
string(APPEND OPTIONS " ${FRAMECAPTURE_FFMPEG_OPTIONS}")
# --- FrameCapture overlay: END --------------------------------------------
